// src/assets/js/console-dungeon.js
export default function initConsoleDungeon() {
  ...
  const game = {
    ...
    rooms: {
      ...
      tresor: {
        ...
        description: "Des coffres brillent dans la pénombre... mais quelque chose veille.",
        visited: false,
        enemies: ["mimic"],
      ...
      boss_room: {
        ...
        description: "Une immense porte s'ouvre sur une salle circulaire. Le boss t'attend.",
        visited: false,
        enemies: ["dragon"],
        items: [],
        exits: { sud: "corridor" }
    ...
    items: {
      potion: { name: "Potion de Soin", effect: "heal", value: 30, desc: "Restaure 30 PV" },
      super_potion: { name: "Super Potion", effect: "heal", value: 60, desc: "Restaure 60 PV" },
      epee_rouille: { name: "Épée Rouillée", effect: "atk", value: 5, desc: "+5 ATK" },
      amulette: { name: "Amulette Mystique", effect: "def", value: 3, desc: "+3 DEF" },
      epee_legendaire: { name: "Épée Légendaire", effect: "atk", value: 15, desc: "+15 ATK" }
    },
    currentEnemy: null,
    inCombat: false
  };
  ...
  function showIntro() {
    ...
    console.log("%c╚═══════════════════════════════════════════╝", styles.title);
    console.log("\n%cBienvenue, aventurier ! Tu te réveilles à l'entrée d'un donjon mystérieux...", styles.info);
    console.log("\n📜 %cCommandes disponibles :", styles.command);
    console.log("  • %cregarde()%c - Observer les alentours", styles.command, "");
    console.log("  • %cstats()%c - Voir tes statistiques", styles.command, "");
    console.log("  • %cinventaire()%c - Vérifier ton inventaire", styles.command, "");
    console.log("  • %cva('direction')%c - Se déplacer (nord, sud, est, ouest)", styles.command, "");
    console.log("  • %cprendre('objet')%c - Ramasser un objet", styles.command, "");
    console.log("  • %cutilise('objet')%c - Utiliser un objet", styles.command, "");
    console.log("  • %cattaque()%c - Attaquer en combat", styles.command, "");
    console.log("  • %csort()%c - Lancer un sort de feu (coûte 20 mana)", styles.command, "");
    console.log("  • %cfuir()%c - Tenter de fuir un combat", styles.command, "");
  ...
  window.inventaire = function() {
    ...
    if (game.player.inventory.length === 0) {
      ...
      game.player.inventory.forEach(item => {
        console.log("  • " + game.items[item].name + " - " + game.items[item].desc);
      });
    }
  };
  ...
  window.va = function(direction) {
    if (game.inCombat) {
      console.log("⚠️  Tu ne peux pas fuir comme ça ! Utilise %cfuir()%c en combat.", styles.command, "");
      return;
    }
    ...
    if (newPos) {
      ...
      console.log("\n🚶 %cTu te déplaces vers le " + direction + "...", styles.info);
      regarde();
    } else {
  ...
  window.prendre = function(itemName) {
    const room = game.rooms[game.player.position];
    const itemKey = Object.keys(game.items).find(k => 
      game.items[k].name.toLowerCase().includes(itemName.toLowerCase())
    );
    
    if (itemKey && room.items.includes(itemKey)) {
      game.player.inventory.push(itemKey);
      room.items = room.items.filter(i => i !== itemKey);
      console.log("✅ %cTu as ramassé : " + game.items[itemKey].name, styles.success);
    } else {
  ...
  window.utilise = function(itemName) {
    ...
    if (itemKey) {
      ...
      if (item.effect === "heal") {
        ...
        console.log("💚 %c+" + item.value + " PV ! PV actuels : " + game.player.hp + "/" + game.player.maxHp, styles.heal);
        game.player.inventory = game.player.inventory.filter(i => i !== itemKey);
      } else if (item.effect === "atk") {
        game.player.atk += item.value;
        console.log("⚔️  %cATK +" + item.value + " ! ATK totale : " + game.player.atk, styles.success);
        game.player.inventory = game.player.inventory.filter(i => i !== itemKey);
      } else if (item.effect === "def") {
        game.player.def += item.value;
        console.log("🛡️  %cDEF +" + item.value + " ! DEF totale : " + game.player.def, styles.success);
        game.player.inventory = game.player.inventory.filter(i => i !== itemKey);
      }
    } else {
  ...
  function startCombat(enemyKey) {
    ...
    console.log("\n⚔️  %c" + game.currentEnemy.name + " apparaît !", styles.enemy);
    console.log("❤️  PV : " + game.currentEnemy.hp + "/" + game.currentEnemy.maxHp);
    console.log("\n💡 %cattaque()%c ou %csort()%c ou %cfuir()", styles.command, "", styles.command, "", styles.command);
  }
  ...
  window.attaque = function() {
    ...
    console.log("   Ennemi : " + game.currentEnemy.hp + "/" + game.currentEnemy.maxHp + " PV");
  ...
  window.sort = function() {
    ...
    game.currentEnemy.hp -= damage;
    console.log("\n🔥 %cBoule de feu ! -" + damage + " PV critiques !", styles.damage);
    console.log("   Ennemi : " + game.currentEnemy.hp + "/" + game.currentEnemy.maxHp + " PV");
    console.log("   Mana restante : " + game.player.mana + "/" + game.player.maxMana);
  ...
  function enemyTurn() {
    ...
    console.log("\n👹 %c" + game.currentEnemy.name + " t'attaque ! -" + damage + " PV", styles.enemy);
  ...
  function levelUp() {
    ...
    console.log("\n✨ %cNIVEAU SUPÉRIEUR ! Niveau " + game.player.level, styles.success);
  ...
  function finalVictory() {
    ...
    console.log("%c║         🏆 VICTOIRE FINALE ! 🏆          ║", styles.success);
    console.log("%c╚═══════════════════════════════════════════╝", styles.success);
    ...
    console.log("  • Salles explorées : " + Object.values(game.rooms).filter(r => r.visited).length + "/5");
  }
